...
public class CommandCreateKey implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
        if(!(sender instanceof Player)) sender.sendMessage("Vous ne pouvez pas exécuter cette commande depuis la console du serveur.");
        else {
            ...
            if (args.length != 0)
                ...
                if(doors.exists(targetedBlock.getType().name())){
                    //Door Locked ?
                    if(doorChecker.isDoorLocked(targetedBlock.getLocation())){player.sendMessage("§cCette porte est déjà verrouillée.");return false;}
                    //Lock Door
